...
CREATE TABLE IF NOT EXISTS study_sessions (
  ...
  topic_id TEXT,                 -- optional: linked concept node_id
  selection_id TEXT,             -- optional: originating quote_id
  intent VARCHAR(50) NOT NULL,      -- 'clarify', 'practice', 'review'
...
CREATE TABLE IF NOT EXISTS study_tasks (
  ...
  task_type VARCHAR(50) NOT NULL,   -- 'clarify', 'define_example', 'explain_back'
  prompt TEXT NOT NULL,
  rubric_json JSONB NOT NULL,       -- scoring criteria
  context_pack_json JSONB NOT NULL, -- excerpts + citations
  compatible_modes JSONB NOT NULL,  -- ['explain', 'typing']
...
CREATE TABLE IF NOT EXISTS study_attempts (
  ...
  score_json JSONB NOT NULL,        -- {grounding: 0.8, coherence: 0.9, ...}
  composite_score FLOAT NOT NULL,
  feedback_text TEXT,
  gap_concepts JSONB DEFAULT '[]'::jsonb,  -- [concept_id, ...]
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS usage_logs (
  ...
  user_id VARCHAR(255) NOT NULL,
  tenant_id VARCHAR(255) NOT NULL,
  action_type VARCHAR(50) NOT NULL,  -- 'voice_session', 'chat_tokens'
...
CREATE TABLE IF NOT EXISTS memory_sync_events (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id VARCHAR(255) NOT NULL,
  source VARCHAR(50) NOT NULL,       -- 'voice', 'knowledge_graph', 'manual'
  memory_id VARCHAR(255),            -- Supermemory internal ID
